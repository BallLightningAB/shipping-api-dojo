...
- Final type-safety review pass: changed `sql.raw('"seed"')` to `sql`${practiceSeeds.seed}`` in the no-op upsert set clause to provide type-safe column reference while preserving the first generated seed on conflicts.
- Twelfth review pass: aligned `buildScenarioPracticeRun` return type with its runtime stripping by changing `Scenario | null` to `Omit<Scenario, "runSeed"> | null`, and matched the arena `currentActiveScenario` state type to the same `Omit<Scenario, "runSeed"> | null` so loader-derived active scenarios flow through without type drift.
- Thirteenth review pass: reworked the `createPracticeSeedId` fallback to produce an RFC 4122-compliant UUID v4 (via the standard version/variant bit masks) and added a unit test that stubs `globalThis.crypto.randomUUID` to drive the fallback path.
