...
The `#11` handoff lives in `specs/archived/completed/issue-11-external-services-setup.md`, with details spread over the different phases in the respective files.
...
- implemented a mixed legacy/family content runtime so migrated and unmigrated lessons can render through the same route API
- converted `rest-1-http-semantics` and `soap-1-envelope-namespaces` to canonical lesson definitions backed by deterministic drill families
- converted `timeout-create-shipment` into a scenario-family run builder with deterministic evidence and ladder steps
- added deterministic seed helpers plus automated tests for stable shuffle, route-seed derivation, and mixed legacy/family runtime behavior
- updated lesson and arena routes to use loader/search-param seeds and reroll actions without hydration-risk randomization in SSR-visible content
- kept progress keyed to canonical family IDs for migrated drills and canonical scenario-family IDs for the scenario exemplar
- enabled signed-in progress write-through after initial server sync so the new canonical progress keys flow through the existing `#11` server-backed contracts
...
- `pnpm lint`
- `pnpm build`
- verified the migrated lesson and arena routes from the local dev server using seeded route requests
