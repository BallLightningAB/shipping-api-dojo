		...
		handlers: {
			POST: async ({ request }) => {
				try {
					...
					// Webhook handlers must not swallow errors silently: Creem retries
					// non-2xx responses, and without captureException a transient DB
					// failure would disappear from the only process output. Returning
					// 500 keeps Creem's retry behavior intact while the error still
					// reaches Sentry once a DSN is configured.
					captureException(error, {
						operation: "creem_webhook",
