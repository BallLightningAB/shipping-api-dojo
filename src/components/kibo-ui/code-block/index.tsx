...
export const CodeBlockHeader = ({
	...
	<div
		className={cn(
			"flex flex-row items-center border-b bg-secondary p-1 text-secondary-foreground font-bold",
			className
		)}
		{...props}
	/>
...
export const CodeBlockFilename = ({
	...
	return (
		<div
			className="flex items-center gap-2 bg-secondary px-4 py-1.5 text-secondary-foreground text-xs"
			{...props}
		>
		...
		<pre className="w-full">
			<code>
				{children
					...
					.split("\n")
					.map((line) => (
						<span className="line" key={line.slice(0, 20)}>
							{line}
						</span>
					))}
			</code>
